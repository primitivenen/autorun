...
for f in ['vehicle.py', 'vehicle_stats.py', 'vehicle_utils.py',
          ...
          'get_energy_n_capacity.py', 'get_geo_stats.py',
          'COL_DEPENDENCY_DICT.py']:
...
COL_NUM_DICT = {1: 'vintype', 2: 'ts', 4: 'veh_charge_st', 5: 'veh_runmode', 6: 'veh_spd', 
                7: 'veh_odo', 8: 'veh_volt', 10: 'veh_curr', 11: 'veh_soc', 
                14: 'veh_acce_pedal', 15: 'veh_brake_pedal', 26: 'eng_consumption',
                32: 'data_batt_sc_volt_highest', 35: 'data_batt_sc_volt_lowest', 
                38: 'data_batt_temp_highest', 41: 'data_batt_temp_lowest',
                72: 'esd_volt', 73: 'esd_curr', 77: 'esd_sc_volt_list',
                78: 'esd_sc_temp_list'}
...
def main(**kwargs):
    ...
    guobiao_hdfs_dir = kwargs.pop('guobiao_hdfs_dir', default_configs['guobiao_hdfs_dir'])
    ...
    for i, d in enumerate(dates):
        ...
        data_file = guobiao_hdfs_dir + 'csv/d={}'.format(d)
        ...
        returncode = run_cmd(['hdfs', 'dfs', '-test', '-e', '/data/guobiao/csv/d={}'.format(d)])
        if returncode:
        ...
        rdd = sc.textFile(data_file).filter(lambda line: len(line.split(',')) in [79])
    ...
    for i, d in enumerate(dates):
        ...
        table_name = 'guobiao_tsp_tbls.{}_stats'.format(freq)
        ts = pd.to_datetime(datetime.now())
        print('{}: exporting {} to Hive table {} ..'.format(ts, stats_file, table_name))
        ...
        # assuming the stats result is for one day only, i.e. the day column contains 
        # unique values of 'd', otherwise the partition would be wrong
        ...
        sql_cmd = """SELECT COUNT(*) FROM {}
                  WHERE {}='{}' """.format(table_name, partition_col, day)
        print(sql_cmd)
        hc.sql(sql_cmd).show()

        sql_cmd = """INSERT OVERWRITE TABLE {}
                  PARTITION ({}='{}')
                  SELECT * FROM update_dataframe""".format(table_name, partition_col, day)
        print(sql_cmd)
        hc.sql(sql_cmd)
