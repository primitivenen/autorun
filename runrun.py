...
def main(**kwargs):
    dates = kwargs.pop('dates', 20150118)  # default to 20150118
    if not isinstance(dates, list):
        dates = [dates]
    freq = kwargs.pop('freq', 'daily')
    stats = kwargs.pop('stats', sorted(list(COL_DEPENDENCY_DICT.keys())))
    customized_module_file = kwargs.pop('customized_module_file', 'veh.zip')
    dest_dir = kwargs.pop('dest_dir', 'ag_stats/')
    ...
    for i, d in enumerate(dates):
        ...
        ts = pd.to_datetime(datetime.now())
        print('{}: exporting {} to Hive table tsp_tbls.{}_stats_trans ..'.format(ts, stats_file, freq))
        ...
        sql_cmd = """delete from tsp_tbls.daily_stats_trans where day='2015-01-18'"""
        print(sql_cmd)
        hc.sql(sql_cmd).show()

        sql_cmd = """SELECT COUNT(*) FROM tsp_tbls.{}_stats_trans
                  WHERE {}='{}' """.format(freq, partition_col, day)
        print(sql_cmd)
        hc.sql(sql_cmd).show()

        sql_cmd = """INSERT INTO TABLE tsp_tbls.{}_stats_trans
                  PARTITION ({}='{}')
